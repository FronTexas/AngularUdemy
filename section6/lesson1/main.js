// TODO THE 'Contact' resource seems to be expired and no longer returning any data
...
app.config(function($httpProvider, $resourceProvider, laddaProvider){
	$httpProvider.defaults.headers.common['Authorization'] = 'Token 3f9ee3beae2186d3b611be9ca184258823207073';
...
app.controller('PersonListController', function ($scope, ContactService, $modal) {
	...
	$scope.showCreateModal = function(){
		...
		$scope.createModal = $modal({
			scope: $scope,
			template: 'templates/model.create.tpl.html',
			show:true
		})
	}
		...
		'loadContacts': function(){
			if (self.hasMore && !self.isLoading){
				...
				Contact.get(params,function(data){
					angular.forEach(data.result ,function(person){
